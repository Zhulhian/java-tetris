...
public class TetrisComponent extends JComponent implements BoardListener {
    ...
    final private Color diveBG;
    final private Color diveFG;
	...
	this.diveBG = new Color(60, 60, 60);
	this.diveFG = new Color(100, 100, 100);
	...
	// Magic constants are RGB values, not worth a fix
	...
	for (int y = 0; y < gameBoard.getHeight(); y++) {
		...
		if (drawFallingAt(x, y)) {
		    ...
		    } else if (gameBoard.getCollisionHandler().equals("dive")) {
			tile.setColor(diveBG);
			sfx.setColor(diveFG);
		    }
		    else {
			tile.setColor(
