...
public class TetrisComponent extends JComponent implements BoardListener {
    ...
    // Not sure why this needs javadoc, it's a constant. Maybe because it's
    // public? Either way, it's pretty self-explanatory in my opinion.
    ...
    // DOS VGA font, used for all text. If not initialized, it will fall back
    // on default font.
	...
	if (gameBoard.isPaused) {
	    ...
	    int centerX = ((gameBoard.getWidth() * SQUARE_SIZE) - ("PAUSE".length() * PAUSE_FONT_SIZE)) / 2 + PAUSE_FONT_SIZE;
	    int centerY = gameBoard.getHeight() * SQUARE_SIZE / 2;
	    sfx.setColor(pauseBG);
	    tile.setColor(fontColor);
	    tile.setFont(pauseFont);

	    // First draw the green box
	    // The PADDING is the space between the text and the box's edges.
	    sfx.fillRect(centerX - PADDING, centerY - PAUSE_FONT_SIZE + PADDING * 2, "PAUSE".length() * SQUARE_SIZE + PADDING, PAUSE_FONT_SIZE + PADDING);
	    // Then write the text.
